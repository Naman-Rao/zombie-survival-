// Import necessary Three.js and custom modules
...
// Fragment shader for sky rendering with gradient effect
const _FS = `
...
/**
 * Main game class for a Hack and Slash style 3D game demo
 * Manages game initialization, rendering, and game loop
 */
class HackNSlashDemo {
  constructor() {
    // Initialize the game when the class is instantiated
    this._Initialize();
  }

  /**
   * Comprehensive initialization method
   * Sets up renderer, scene, camera, lighting, and game entities
   */
  _Initialize() {
    // Configure WebGL renderer with advanced settings
    ...
    // Initialize entity management and spatial grid
  ...
  /**
   * Load UI controllers for the game
   */
  _LoadControllers() {
  ...
  /**
   * Create sky rendering with hemisphere light and shader-based gradient
   */
  _LoadSky() {
    ...
    // Create sky sphere with custom shader material
  ...
  /**
   * Procedurally generate clouds in the scene
   */
  _LoadClouds() {
    for (let i = 0; i < 20; ++i) {
  ...
  /**
   * Procedurally generate various types of trees and foliage
   */
  _LoadFoliage() {
    for (let i = 0; i < 100; ++i) {
      const names = [
  ...
  /**
   * Load player, NPCs, weapons, and other game entities
   */
  _LoadPlayer() {
    // Shared parameters for entities
    const params = {
      camera: this._camera,
      scene: this._scene,
    };

    // Create level up spawner entity
    ...
    // Create main player entity with multiple components
    ...
    // Add initial weapons to player inventory
    player.Broadcast({
        topic: 'inventory.add',
        value: axe.Name,
        added: false,
    ...
    // Spawn multiple monster NPCs
    this._SpawnMonsters();
  }

  /**
   * Spawn multiple monster NPCs with varied models
   */
  _SpawnMonsters() {
    const monsters = [
      {
    ...
    // Spawn 50 random monsters across
    for (let i = 0; i < 50; ++i) {
      const monsters = [
        {
          resourceName: 'Ghost.fbx',
